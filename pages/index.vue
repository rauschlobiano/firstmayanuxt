  ...
  <v-app>
    <div data-app>
        ...
        <h1 class="mx-4" style="color: gray">|</h1>
          ...
          <v-tooltip bottom style="z-index: 1000;">
            <template v-slot:activator="{ on, attrs }">
              <v-btn @click="logout" icon v-bind="attrs"  v-on="on" v-if="loggedin">
      ...
      <v-row class="mt-6">
          ...
          <v-spacer></v-spacer>
          <v-col cols="4">
            <v-card elevation="5" style="z-index: 10" max-width="90%" height="400" v-if="!loggedin">
              <v-row class="mx-auto">
                <v-col>
...
export default {
  ...
  methods: {
    ...mapMutations(["updateUserInfo", "updateLegitUser"]),
    ...
    async login() {
      ...
      let res = await this.callApi("POST", "/userinfo/login", {uname: this.uname, passw: this.passw});
        ...
        if(res.data.userData){
          ...
          this.updateUserInfo(res.data.userData)
        }
    },
